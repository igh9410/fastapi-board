...
def create_post_crud(
    db: Session, board_id: UUID, post: schemas.PostCreate, author_id: UUID
) -> Union[models.Post, Tuple[None, str]]:
...
def delete_post_crud(
    db: Session, board_id: UUID, post_id: UUID, author_id: UUID
) -> None:
    ...
    ):  # If the user is not the author of the post, delete is not allowed
        logging.info(f"User {author_id} is not the author of post {post_id}")
        raise ValueError("Only the post's author can delete it")

    delete_stmt = delete(models.Post).where(models.Post.id == post_id)
...
def get_post_list_crud(
    db: Session, board_id: UUID, user_id: UUID, page_number: int, page_size: int
) -> Union[schemas.PostList, Tuple[None, str]]:
    ...
    try:
        ...
        posts_stmt = (
            ...
            )  # Ordering by created_at in descending order, fetching the latest posts first
            .offset(offset)
            .limit(page_size)
        )
        ...
        total_posts_stmt = select(func.count(models.Post.id)).where(
...
def is_board_accessible(
    db: Session, board_id: UUID, user_id: UUID
) -> bool:  # Check if board is public or user is the creator
    ...
    if (
        db_board.creator == user_id
    ):  ## If the user is the creator of the board, they can view it
        return True

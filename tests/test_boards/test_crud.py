...
def test_create_board_crud():
    ...
    with patch('app.boards.crud.create_board_crud', return_value=mock_board) as mock_create_board_crud:
        result = crud.create_board_crud(mock_session, board_data, user_id)

        mock_create_board_crud.assert_called_once_with(mock_session, board_data, user_id)
...
def test_can_update_board_crud(): ## Should be able to update board if user is creator
    ...
    mock_session.execute.return_value.scalars.return_value.first.return_value = mock_board
...
def test_cannot_update_board_crud():
    ...
    result, message = crud.update_board_crud(mock_session, board_id, board_data, user_id_2)
...
def test_can_get_public_board_crud(): ## Should be able to get board if user is not creator but board is public
    ...
    mock_session = Mock()
    user_id = uuid4() # The user ID of the original creator
    user_id_2 = uuid4() # The user ID of the user trying to get the board, different from creator
    board_id = uuid4()
